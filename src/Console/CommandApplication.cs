...
namespace MaSch.Console
{
    public class CommandApplication
    {
        ...
        public CommandApplication(IConsoleService console) : this(console, null) { }
        ...
        public void ShowHelp(string[] prms)
        {
            if (string.IsNullOrEmpty(prms[0]))
            {
                ...
                {
                    _console.WriteLine($"- {string.Join(" | ", command.Names)}{NewLine}    {command.Description.Replace("\r", "").Replace("\n", $"{NewLine}    ")}");
                }
            }
            else
            {
