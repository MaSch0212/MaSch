...
public static class AsyncAdapter
{
    ...
    private class DataReaderAdapter
    {
        ...
        internal DataReaderAdapter(Type type)
        {
            ...
            getFieldValueAsyncToken = type.GetRuntimeMethod("GetFieldValueAsync", new[] { typeof(int), typeof(CancellationToken) });
        }
    ...
    public static Task OpenAsync(this IDbConnection connection)
    {
        ...
        if (connection is DbConnection dbConnection) return dbConnection.OpenAsync();
    ...
    {
        if (connection == null) throw new ArgumentNullException(nameof(connection));
        if (connection is DbConnection dbConnection) return dbConnection.OpenAsync(token);
        return ConnectionAdapters.GetOrAdd(connection.GetType(),
            type => new ConnectionAdapter(type))
    ...
    {
        if (command == null) throw new ArgumentNullException(nameof(command));
        if (command is DbCommand dbCommand) return dbCommand.ExecuteReaderAsync().ContinueWith<IDataReader>(t => t.Result);
        return CommandAdapters.GetOrAdd(command.GetType(),
            type => new CommandAdapter(type))
    ...
    {
        if (command == null) throw new ArgumentNullException(nameof(command));
        if (command is DbCommand dbCommand) return dbCommand.ExecuteReaderAsync(token).ContinueWith<IDataReader>(t => t.Result, token);
        return CommandAdapters.GetOrAdd(command.GetType(),
            type => new CommandAdapter(type))
    ...
    {
        if (command == null) throw new ArgumentNullException(nameof(command));
        if (command is DbCommand dbCommand) return dbCommand.ExecuteReaderAsync(commandBehavior).ContinueWith<IDataReader>(t => t.Result);
        return CommandAdapters.GetOrAdd(command.GetType(),
            type => new CommandAdapter(type))
    ...
    {
        if (command == null) throw new ArgumentNullException(nameof(command));
        if (command is DbCommand dbCommand) return dbCommand.ExecuteReaderAsync(commandBehavior, token).ContinueWith<IDataReader>(t => t.Result, token);
        return CommandAdapters.GetOrAdd(command.GetType(),
            type => new CommandAdapter(type))
    ...
    public static Task<T> GetFieldValueAsync<T>(this IDataReader reader, int ordinal)
    {
        ...
        if (reader is DbDataReader dataReader) return dataReader.GetFieldValueAsync<T>(ordinal);
    ...
    {
        if (reader == null) throw new ArgumentNullException(nameof(reader));
        if (reader is DbDataReader dataReader) return dataReader.GetFieldValueAsync<T>(ordinal, token);
        return DataReaderAdapters.GetOrAdd(reader.GetType(),
            type => new DataReaderAdapter(type))
    ...
    {
        if (reader == null) throw new ArgumentNullException(nameof(reader));
        if (reader is DbDataReader dataReader) return dataReader.IsDBNullAsync(ordinal);
        return DataReaderAdapters.GetOrAdd(reader.GetType(),
            type => new DataReaderAdapter(type))
    ...
    {
        if (reader == null) throw new ArgumentNullException(nameof(reader));
        if (reader is DbDataReader dataReader) return dataReader.IsDBNullAsync(ordinal, token);
        return DataReaderAdapters.GetOrAdd(reader.GetType(),
            type => new DataReaderAdapter(type))
    ...
    {
        if (reader == null) throw new ArgumentNullException(nameof(reader));
        if (reader is DbDataReader dataReader) return dataReader.NextResultAsync();
        return DataReaderAdapters.GetOrAdd(reader.GetType(),
            type => new DataReaderAdapter(type))
    ...
    {
        if (reader == null) throw new ArgumentNullException(nameof(reader));
        if (reader is DbDataReader dataReader) return dataReader.NextResultAsync(token);
        return DataReaderAdapters.GetOrAdd(reader.GetType(),
            type => new DataReaderAdapter(type))
    ...
    {
        if (reader == null) throw new ArgumentNullException(nameof(reader));
        if (reader is DbDataReader dataReader) return dataReader.ReadAsync();
        return DataReaderAdapters.GetOrAdd(reader.GetType(),
            type => new DataReaderAdapter(type))
                .ReadAsync(reader);
    }
    ...
    {
        if (reader == null) throw new ArgumentNullException(nameof(reader));
        if (reader is DbDataReader dataReader) return dataReader.ReadAsync(token);
        return DataReaderAdapters.GetOrAdd(reader.GetType(),
            type => new DataReaderAdapter(type))
