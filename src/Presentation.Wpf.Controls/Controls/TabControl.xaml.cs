...
namespace MaSch.Presentation.Wpf.Controls
{
    public class TabControl : System.Windows.Controls.TabControl
    {
        ...
        public static readonly DependencyProperty AnimationDurationSecondsProperty =
            DependencyProperty.Register("AnimationDurationSeconds", typeof(double), typeof(TabControl), new PropertyMetadata(0.2D));
        public static readonly DependencyProperty IsPreviewEnabledProperty =
            DependencyProperty.Register("IsPreviewEnabled", typeof(bool), typeof(TabControl), new PropertyMetadata(true));
        public static readonly DependencyProperty PreviewDelayProperty =
            DependencyProperty.Register("PreviewDelay", typeof(int), typeof(TabControl), new PropertyMetadata(1000));
        public static readonly DependencyProperty IsNavigationPartVisibleProperty =
            DependencyProperty.Register("IsNavigationPartVisible", typeof(bool), typeof(TabControl), new PropertyMetadata(true));
            ...
            eventTo.MouseEnter += async (s, ex) =>
            {
                if (IsPreviewEnabled && !item.IsSelected)
                {
                    ...
                    if (item.IsMouseOver && !item.IsSelected)
                    {
                        ...
                        while (item.IsMouseOver && !item.IsSelected) { await Task.Delay(50); }
        ...
        {
            if (_previewContent != null && _currentContent != null)
            {
                var be = _currentContent.Effect as BlurEffect;
                var sc = _currentContent.RenderTransform as ScaleTransform;
                if (be != null && sc != null)
                {
        ...
        {
            var lastContent = GetTemplateChild("MaSch_LastContent") as ContentPresenter;
            _currentContent = GetTemplateChild("PART_SelectedContentHost") as ContentPresenter;
            if (lastContent != null && _currentContent != null)
            {
        ...
        {
            var underline = GetTemplateChild("MaSch_Underline") as Border;
            var stackpnl = GetTemplateChild("MaSch_TabsInner") as StackPanel;
            if (underline != null && stackpnl != null)
            {
        ...
        {
            var btnPrev = GetTemplateChild("MaSch_PrevTab") as Button;
            var btnNext = GetTemplateChild("MaSch_NextTab") as Button;
            if (btnPrev != null)
            {
                SelectionChanged += (s, e) =>
                {
